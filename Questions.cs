...
public class Questions : MonoBehaviour {
	...
	void OnGUI()
	{
		...
		if (j > 0)
		{
			...
			scrollposition = GUI.BeginScrollView(new Rect(0, 0, Screen.width, Screen.height), scrollposition, new Rect(0, 0, Screen.width, Screen.height*(float) (j+1)));
			...
			//Add save all button.
			if(GUI.Button(new Rect((float)4.5*widthUnit, 8*j*heightUnit+Screen.height, 2*widthUnit, 1*heightUnit), "Save Answers", saveButton) && toggleTxt) {
				//write file
	...
	void writeToFile(string[] results, string[,] questions, int length) {
		...
		if (data != null) {
			...
			gameData gd = (gameData) data.GetComponent("gameData");
			string title ="";
			string fileName = gd.unique_id + "_questions.txt";
