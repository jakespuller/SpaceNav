				...
				if (timeLeft <= 0.0f) {
						...
						if (popup == false) {
								if (!gd.scriptedInstance) {
										...
										//if questions exist for current game level goto that, else 
										if (gd.gameLevel <= gd.maxGameLevel) {
											//if no questins goto question level
												StartCoroutine (pauseAndLoadNewLevel (3, 2));
										} else {
											//if question exists goto question level
												StartCoroutine (pauseAndLoadNewLevel (3, 2));
										}
								} else {
